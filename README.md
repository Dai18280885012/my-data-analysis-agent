...
一个面向企业内部场景的本地 Agent 项目：用户用自然语言提问，系统自动选择数据分析或制度知识库能力，返回带真实数据依据或文档来源的中文答案。全程本地运行（Ollama + Chroma），数据不出内网。

## 核心亮点

- **LangGraph 受控 Agent**：意图识别 → 工具调用 → 结果校验 → 条件路由 → 生成回答 的完整工作流；意图白名单 + 工具参数范围校验 + 结果完整性校验 + 失败自动兜底。
- **工具契约化设计**：工具统一输入输出契约，配套契约测试，保证 Agent 链路可校验、可回归。
- **RAG 混合检索**：向量检索（bge-small-zh-v1.5 + Chroma）+ 关键词评分双路召回，句子感知切分，资料不足时明确拒答而非编造。
- **真实数据处理**：Pandas 清洗 54 万+ 行真实交易数据（取消订单识别剔除、负值过滤、字段校验），SQLite 持久化。
- **工程与安全**：参数化 SQL、`data_coverage` 数据覆盖范围提示、.env 配置管理（无硬编码密钥）、Docker Compose 一键部署。
...
![本地数据分析 Agent 界面](docs/screenshot.png)

Streamlit 界面实际运行效果：用户自然语言提问「哪个国家卖得最好？」，Agent 完成意图识别 → 工具调用 → 结果校验 → 回答生成，返回带数据依据的结论，并展示国家销售额排行柱状图与工具调用记录。

## 系统架构

```mermaid
flowchart TD
    U[用户浏览器 / Streamlit] --> D[Dify Chatflow / 入口]
    D --> C{问题分类器}
    C -->|公司制度问题| R[RAG 知识库<br/>Chroma + bge-small-zh-v1.5]
    R --> L[本地 LLM<br/>Ollama DeepSeek/Qwen]
    C -->|销售分析问题| F[FastAPI 服务]
    F --> P[Pandas / SQLite<br/>受控查询]
    P --> L
    C -->|不支持问题| FB[兜底回复]
    L --> A[带数据依据的中文回答]
```

LangGraph 受控 Agent 内部链路：

```mermaid
flowchart LR
    Q[自然语言问题] --> I[意图识别<br/>白名单校验]
    I --> T[工具调用<br/>参数范围校验]
    T --> V[结果校验<br/>完整性检查]
    V -->|通过| G[生成回答<br/>结论+数据依据]
    V -->|失败| E[失败兜底<br/>说明原因与可用能力]
```
...
```powershell
# 1. 启动 FastAPI 数据分析服务（项目根目录）
cd my-data-analysis-agent
python -m uvicorn api:app --host 0.0.0.0 --port 8000

# 2. 另开终端，启动 Dify（如使用 Docker 部署）
cd dify/docker
docker compose up -d
```
